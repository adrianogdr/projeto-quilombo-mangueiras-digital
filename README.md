# 🌿 Projeto Digital: Quilombo Mangueiras

> **Solução de Inclusão Digital e E-commerce Social** para a comunidade Quilombola Mangueiras (Belo Horizonte/MG).

<img width="1132" height="179" alt="image" src="https://github.com/user-attachments/assets/7b9f282e-5b92-4bed-9cdc-1340b5e5f985" />

## 🎯 O Desafio
A comunidade Quilombo Mangueiras possui uma produção rica de agroecologia e artesanato, mas carecia de uma presença digital centralizada para divulgar sua história, lutas e facilitar a comercialização de seus produtos para fora da comunidade.

## 💡 A Solução
Desenvolvimento de um ecossistema digital acessível e de baixo custo de manutenção, permitindo que a própria comunidade gerencie o conteúdo futuramente.

### Tecnologias Utilizadas (Low-Code)
* **Front-end:** Google Sites (Interface responsiva e leve).
* **Coleta de Dados:** Google Sites - Codigo Incorporado (Para pedidos e contato).
* **Gestão:** Google Sheets (Integração para controle de pedidos).
* **Design:** Customização de UI focado na identidade visual afro-brasileira.

## 🚀 Funcionalidades
- [x] **Landing Page Institucional:** Apresentação da história e cultura do Quilombo.
- [x] **Catálogo Digital:** Vitrine virtual dos produtos agroecológicos.
- [x] **Sistema de Pedidos:** Formulário digital para encomendas diretas.
- [x] **Responsividade:** Acesso otimizado para celulares (principal meio de acesso da comunidade).

## 🔗 Links
- **Acesse o Site:** [https://sites.google.com/view/quilombo-mangueiras](https://sites.google.com/view/quilombo-mangueiras)

---
*Desenvolvido por Adriano Gonçalves*
